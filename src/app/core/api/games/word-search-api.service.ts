...
import { CellCoordinate } from "../../../features/games/word-search/models/cell-coordinate.model";
import { ShapedWordSearchQuery } from "../../../features/games/word-search/models/shaped-word-search-query.model";
import { CustomShapedWordSearchQuery } from "../../../features/games/word-search/models/custom-shaped-word-search-query.model";
...
export class WordSearchApiService {
  ...
  private readonly apiUrlShaped = 'http://localhost:8080/api/v1/games/shaped-word-search';
  private readonly apiUrlCustomShaped = 'http://localhost:8080/api/v1/games/custom-shaped-word-search';
  ...
  public generateGame(query: WordSearchQuery): Observable<WordSearchResponse> {
    ...
    params = this.appendFilters(params, query.filters);
  ...
  public generateShapedGame(
    query: ShapedWordSearchQuery
    ...
    params = this.appendBlockedCells(params, query.settings.blockedCells);
    ...
    return this.http.get<WordSearchResponse>(
      `${this.apiUrlShaped}/generate`,
      { params }
    );
  }

  public generateCustomShapedGame(
    query: CustomShapedWordSearchQuery
  ): Observable<WordSearchResponse> {
    let params = new HttpParams()
      .set('rows', query.settings.rows)
      .set('cols', query.settings.cols)
    ...
    return this.http.get<WordSearchResponse>(
      `${this.apiUrlCustomShaped}/generate`,
      { params }
    );
  }
  ...
  private appendFilters(
    ...
    if (filters.includeCategories?.length) {
      params = params.set(
        'includeCategories',
        filters.includeCategories.join(',')
      );
    }
